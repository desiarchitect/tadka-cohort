...
> New here? Read [`README.md`](README.md). Two apps + Kafka. Kafka is **off** when `Kafka:BootstrapServers` is unset (so `dotnet test` needs no broker).

**Every command block below is given twice — bash first, PowerShell second.** They are not the same syntax with `curl` swapped for `curl.exe`: bash's `VAR=value`, `$(...)`, `| sed`, and `for`/`until` loops are not valid PowerShell, and copy-pasting them into a plain PowerShell prompt will fail outright, not just print a warning. Use whichever block matches the shell you're actually typing into — Git Bash (already on this machine) for the bash blocks, Windows PowerShell for the PowerShell ones. Both were run live end-to-end on this branch before this runbook was written this way.
...
```
From here on, wherever a bash block does `curl ... | sed ...` to place an order and grab its id, the PowerShell equivalent is:
```powershell
$order = (Invoke-RestMethod -Uri http://localhost:5224/api/v1/orders -Method Post -ContentType "application/json" -Body $BODY).id
```
and wherever a bash block checks status with `curl .../orders/$ORDER | sed ...`, the PowerShell equivalent is:
```powershell
(Invoke-RestMethod -Uri "http://localhost:5224/api/v1/orders/$order").status
```
Both verified live — `Invoke-RestMethod` is genuinely simpler here than the bash `curl | sed` dance, since it parses JSON natively instead of regex-extracting a field from raw text.
...
docker exec tadka-postgres psql -U tadka -d tadka -c "SELECT \`"Topic\`", \`"ProcessedAt\`" IS NOT NULL AS sent FROM ordering.outbox_messages ORDER BY \`"CreatedAt\`" DESC LIMIT 3;"
```
> **A PowerShell-specific gotcha, worth knowing once:** every `docker exec ... psql -c "..."` command in this runbook has double-quoted Postgres column names inside it (`"Topic"`, `"CreatedAt"`, etc. — this schema is case-sensitive EF Core-generated SQL, not optional styling). Plain `\"` escaping, which works in bash, gets silently stripped by PowerShell before it ever reaches `docker.exe` — verified live, it just breaks the query with a "column does not exist" error. The `` \`" `` sequence (backslash *then* backtick-quote) is what actually survives PowerShell's argument passing to a native executable; every PowerShell `psql` block below uses it.
...
(Invoke-RestMethod -Uri "http://localhost:5224/api/v1/orders/$order").status   # Created (pending) — NOT lost
```
...
**How we identified it** — don't take "it's fine" on faith; look at the consumer-group lag directly. The message is sitting in Kafka, unconsumed. This `docker exec` line has no bash-specific syntax, so it's identical in either shell:
```
...
FIX — restart the Payment service (identical either shell). It reconnects to Kafka, rejoins the `tadka-payment` consumer group, and resumes from its **last committed offset** — it does not need to be told what it missed, the broker already knows:
```
...
```
```powershell
(Invoke-RestMethod -Uri "http://localhost:5224/api/v1/orders/$order").status   # Confirmed — caught up
docker exec tadka-kafka /opt/kafka/bin/kafka-consumer-groups.sh --bootstrap-server localhost:9092 --describe --group tadka-payment   # LAG 0
```
...
(Invoke-RestMethod -Uri "http://localhost:5224/api/v1/orders/$order").status   # Cancelled (compensating action via payment-results=Failed)
```
...
```
`grep` isn't a native PowerShell cmdlet, so here's the `Select-String` equivalent — `-Exclude` on `Get-ChildItem` only filters filenames, not directories, so the `Migrations/` exclusion needs an explicit path filter instead:
```powershell
Get-ChildItem -Path src\Tadka.Api -Recurse -Include *.cs | Where-Object { $_.FullName -notmatch '\\Migrations\\' } | Select-String -Pattern "FakePaymentGateway|PaymentDbContext|Domain.Payments|IPaymentClient" | Where-Object { $_.Line -notmatch '^\s*//' }   # nothing — Ordering talks to Payment ONLY via Kafka events
```
No hits means Ordering has zero compile-time knowledge of Payment's gateway, database, or even the `IPaymentClient` interface Day 8 introduced for the HTTP bridge — that interface is gone entirely, replaced by "publish an event and react to another one." Run the full suite to confirm none of this broke anything the earlier days already covered (identical either shell):
```
...
BREAK — inject a malformed message directly onto the `order-placed` topic. These `.ps1` scripts always run through PowerShell under the hood — from a plain Windows PowerShell prompt, run them directly; from Git Bash, invoke `powershell.exe -File`. Neither needs PowerShell 7/`pwsh`, which may not be installed on your machine:
```bash
powershell.exe -File scripts/inject-poison.ps1 -Mode Malformed             # syntactically invalid JSON
```
```powershell
...
LAG tadka-payment | Select-String order-placed               # LAG 1 while retrying, then 0 once it's DLQ'd
```
...
Read the DLQ to confirm the poisoned payload landed there intact (no bash-specific syntax, identical either shell):
```
...
```bash
powershell.exe -File scripts/inject-poison.ps1 -Mode SafeExtraField          # an EXTRA unknown field — processes fine, no code change needed
...
**A trap worth knowing about the tool itself, verified live:** `kafka-consumer-groups.sh --reset-offsets` always exits `0`, even when it prints `Error: ... group ... is still active` and does nothing. A naive `until docker exec ...; do ...; done` (relying on the exit code) would treat that error as success on the very first try and stop waiting — checked live in both bash and PowerShell, confirmed broken both times. Check the actual output text instead of the exit code:
```bash
...
```
```powershell
docker exec tadka-payment-db psql -U tadka -d tadka_payment -c "SELECT count(*) FROM payment.payments;"   # note this BEFORE restarting — identical, no double-quoted columns here
```
Restart the Payment service — with its offset reset to earliest, it will re-consume **every** `order-placed` message that has ever been published to the topic, from the very first order of the day. Give it the usual ~15-20 seconds to fully come up and finish reprocessing before checking anything (identical either shell):
```
...
- **`inject-poison.ps1` (or any `.ps1` script here) says "command not found" or "pwsh not recognized":** in PowerShell, run it as `.\scripts\inject-poison.ps1 ...`; from Git Bash, as `powershell.exe -File scripts/inject-poison.ps1 ...`. Either way, none of this repo's scripts need PowerShell 7/`pwsh`, which may not be installed on your machine at all.
