...
├── day-03-api-primitives/
    ...
    └── grpc-vs-rest-toy/
        ...
        └── RUN-AND-TEST.md
└── day-04-locking/
    └── locking-toy/           # FOR UPDATE block / NOWAIT / SKIP LOCKED (Day 4 Saturday)
```

**When to run (class, not “if ahead”):**

| Toy | Run on | Do not run on |
|---|---|---|
| `cursor-pagination-toy` | **Day 5** (deep pagination / `GET /orders/history`) | Day 3, Day 4 |
| `locking-toy` | **Day 4** (pessimistic engine room) | Day 3 |
| `grpc-vs-rest-toy` | extras only | Saturday Day 3/4 spine |
