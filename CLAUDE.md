...
The .NET 10 codebase students clone and run. This branch is **Day 2**: domain model + schema-per-domain + ADR-003/008, plus Day 1's liveness `/health` and `/health/ready`. Later `day-NN` branches unlock as the cohort moves.
...
- First `/health/ready` after `dotnet run` is ~500–800ms (EF/Npgsql warm-up); hit it twice.
...
`day-01` = scaffold + liveness `/health` + ADR-001/002. `day-02` = domain model + schemas + `/health/ready` + ADR-003/008. Later `day-NN` branches add only what that day earned. This public repo does not carry `main`.
