...
## Branches / tags → days
`day-01` ✅ = scaffold + `/health` + ADR-001/002. `day-02` ✅ = domain model + DbContext + schema-per-domain. `day-03` ✅ = full REST API `/api/v1` (14 endpoints) + order state machine. `day-04` ✅ = order-flow hardening (idempotency, optimistic concurrency `xmin`→409, in-process domain events, Testcontainers tests; 19/19 green). `day-05` ✅ = scaling: performance indexes + tuned connection pool + streaming read replica with EF read/write split (`docker-compose` primary 5432 / replica 5433; 19/19 green). `day-06` ✅ = Redis cache-aside + single-flight stampede lock + delete-on-write + SSE over Redis pub/sub (`StackExchange.Redis` behind `ICacheService`; Redis down → menu 200, SSE 503; leftover ADRs 047–051 not Sunday lecture). `day-07` ✅ = payment brownout: **Polly** timeout + bulkhead (ADR-021), **MediatR** Payment module with own `PaymentDbContext` / `payment` schema / migration history (ADR-022), **async payment** off the request path (ADR-023); env levers `Payment:Mode` Sync/Async/Off + gateway Fast/Slow/Failing; brownout **9.2s → 2.1s → ms**. Each `day-NN` branches from the previous. Keep `main`/`week-*` intact.
